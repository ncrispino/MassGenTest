...
New ``massgen logs`` CLI command for run log analysis with export options (JSON/CSV). Gemini 3 Flash model support. Enhanced config builder with context paths wizard. Web UI "Open in Browser" button. Fixed web search call message preservation, Claude Code tool permissions, and orchestrator timeout handling.
